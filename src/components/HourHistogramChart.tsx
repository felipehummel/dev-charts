...
const HourHistogramChartComponent: FC<HourHistogramChartProps> = ({ title, data, users }) => {
  const theme = useTheme();
  
  // Calcular o total geral para exibir no título
  const totalCount = useMemo(() => {
    return data.reduce((sum, hour) => sum + (hour.total || 0), 0);
  }, [data]);
  ...
  const transformedData = useMemo(() => {
    return data.map(item => {
      ...
      // Adicionar as propriedades dos usuários na ordem correta
        ...
        <Box sx={{ height: 400, width: '100%' }}>
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              />
              <YAxis 
                label={{ value: 'Número de Commits', angle: -90, position: 'insideLeft' }}
              />
              <Tooltip 
...
export const HourHistogramChart = memo(HourHistogramChartComponent);

export default HourHistogramChart; 